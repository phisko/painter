...
struct EntityHighlightSystem {
	...
	static inline putils::NormalizedColor SELECTED_COLOR;
	static inline float SELECTED_INTENSITY = 2.f;

	static inline putils::NormalizedColor HOVERED_COLOR;
	static inline float HOVERED_INTENSITY = 1.f;
};

EXPORT void loadKenginePlugin(void * state) noexcept {
	kengine::pluginHelper::initPlugin(state);
	EntityHighlightSystem::init();
}
